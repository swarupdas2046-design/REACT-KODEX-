  ...
  return (
    ...
    <div className="min-h-screen relative flex flex-col items-center justify-center p-4 font-sans text-slate-800 bg-[radial-gradient(ellipse_at_top_left,_var(--tw-gradient-stops))] from-blue-50 via-slate-50 to-white">
      ...
      <div className="z-10 w-full max-w-100 bg-white rounded-[20px] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100 p-8">
        <h2 className="text-xl font-bold text-slate-900">Secure Sign Up</h2>
        ...
        <form className="space-y-5" onSubmit={handleSubmit(HandleRegisterFormSubmit)}>
            <InputField register={register} rules={{required:'Name is required'}} error={errors.name} Name='name'  type="text" placeholder="Swarup Das" label="Full Name"/>
          {/* Email Input */}
        ...
        <p className="text-center mt-4">Already have an account <span onClick={() => navigate('/')} className="text-[#0f4ed0] hover:text-blue-800 transition-colors cursor-pointer">Sign Up</span></p>
